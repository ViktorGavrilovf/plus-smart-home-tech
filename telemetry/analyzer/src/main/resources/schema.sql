CREATE SCHEMA IF NOT EXISTS analyzer_store;

-- создаём таблицу scenarios
CREATE TABLE IF NOT EXISTS analyzer_store.scenarios (
...
CREATE TABLE IF NOT EXISTS analyzer_store.sensors (
    id VARCHAR PRIMARY KEY,
    hub_id VARCHAR
);

-- создаём таблицу conditions
CREATE TABLE IF NOT EXISTS analyzer_store.conditions (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    type VARCHAR,
    operation VARCHAR,
    value INTEGER
...
CREATE TABLE IF NOT EXISTS analyzer_store.actions (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    type VARCHAR,
    value INTEGER
);
...
CREATE TABLE IF NOT EXISTS analyzer_store.scenario_conditions (
    scenario_id BIGINT REFERENCES analyzer_store.scenarios(id),
    sensor_id VARCHAR REFERENCES analyzer_store.sensors(id),
    condition_id BIGINT REFERENCES analyzer_store.conditions(id),
...
CREATE TABLE IF NOT EXISTS analyzer_store.scenario_actions (
    scenario_id BIGINT REFERENCES analyzer_store.scenarios(id),
    sensor_id VARCHAR REFERENCES analyzer_store.sensors(id),
    action_id BIGINT REFERENCES analyzer_store.actions(id),
...
'
    BEGIN
        IF (SELECT hub_id FROM analyzer_store.scenarios WHERE id = NEW.scenario_id) != (SELECT hub_id FROM analyzer_store.sensors WHERE id = NEW.sensor_id) THEN
...
CREATE OR REPLACE TRIGGER tr_bi_scenario_conditions_hub_id_check
    BEFORE INSERT ON analyzer_store.scenario_conditions
    FOR EACH ROW
...
CREATE OR REPLACE TRIGGER tr_bi_scenario_actions_hub_id_check
    BEFORE INSERT ON analyzer_store.scenario_actions
    FOR EACH ROW
EXECUTE FUNCTION check_hub_id();