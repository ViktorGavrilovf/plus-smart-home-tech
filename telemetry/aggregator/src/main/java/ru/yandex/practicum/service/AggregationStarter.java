...
public class AggregationStarter {
    ...
    public void start() {
        log.info("Запуск Aggregator. Подписка на топик: {}", sensorsTopic);
        ...
        try (KafkaConsumer<String, SensorEventAvro> consumer = new KafkaConsumer<>(consProps);
            ...
            while (true) {
                ...
                for (ConsumerRecord<String, SensorEventAvro> record : records) {
                    ...
                    log.debug("Получено событие от сенсора {} в хабе {}", event.getId(), event.getHubId());
                    ...
                    updatedSnap.ifPresent(snapshot -> {
                        ...
                        log.info("Отправлен обновлённый снапшот хаба {} в топик {}", snapshot.getHubId(), snapshotsTopic);
                    });
                }
            ...
            log.info("Завершаем работу и закрываем ресурсы");
        }
    }
}
